...
class TaskServiceImpl(
...
) : CreatedByUserEntityServiceImpl<Task>(), TaskService {

    private val collection: CoroutineCollection<Task> = database.getCollection(
        DBCollection.TASK.collectionName
    )

    private val notFoundException = EntityNotFoundException(DBCollection.TASK.entityName)
    ...
    override suspend fun create(userId: ObjectId, projectId: ObjectId, dto: CreateTaskDto): Task {
        val task = Task(
            ...
            status = dto.status,
            expectedTime = SimpleDateFormat().parse(dto.expectedTime)
        )
    ...
    override suspend fun updateStatus(id: ObjectId, status: Status): Task {
        return collection
            .findOneAndUpdate(
                filter = Task::_id eq id,
                update = setValue(Task::status, status.name),
                options = FindOneAndUpdateOptions().returnDocument(ReturnDocument.AFTER)
            )
            ?: throw notFoundException
    }
    ...
    override suspend fun updatePriority(id: ObjectId, priority: Priority): Task {
        return collection
            .findOneAndUpdate(
                filter = Task::_id eq id,
                update = setValue(Task::priority, priority.name),
                options = FindOneAndUpdateOptions().returnDocument(ReturnDocument.AFTER)
            )
            ?: throw notFoundException
    }
    ...
    override suspend fun updateBlockedBy(id: ObjectId, blockedBy: List<ObjectId>): Task {
        return collection
            .findOneAndUpdate(
                filter = Task::_id eq id,
                update = setValue(Task::blockedBy, blockedBy.toSet()),
                options = FindOneAndUpdateOptions().returnDocument(ReturnDocument.AFTER)
            )
            ?: throw notFoundException
    }
    ...
    override suspend fun updateTags(id: ObjectId, tags: List<ObjectId>): Task {
        return collection
            .findOneAndUpdate(
                filter = Task::_id eq id,
                update = setValue(Task::tags, tags.toSet()),
                options = FindOneAndUpdateOptions().returnDocument(ReturnDocument.AFTER)
            )
            ?: throw notFoundException
    }
    ...
    override suspend fun deleteById(id: ObjectId): String {
        return collection.findOneAndDelete(Task::_id eq id)?._id?.toString()
            ?: throw notFoundException
    }
